...
- Active overload (multiple actives, no recovery time)
- Recent in-clinic procedures (peels, microneedling, laser)
- Compromised lifestyle (poor sleep, nutrient deficiency, chronic stress)
...
- Photodamage (chronic UV breaks down barrier lipids)
- Ageing (intrinsic decline in barrier function from 40+)
- Persistent inflammatory conditions (chronic rosacea, severe acne)

Recoverable in **3–6 months** with sustained barrier-repair protocol; some structural changes are partially reversible only.
...
Disrupt the ratio and the matrix loses cohesion. Restore the ratio and the barrier rebuilds.
...
The most common barrier-crash pattern: client gets excited about results, adds a second active, then a third, then a peel, all within 4-6 weeks. Barrier collapses. Skin worse than starting position.
...
- BHA (morning) + retinoid (night) + barrier support both ends
- Vitamin C (morning) + retinoid (night) + SPF
- Azelaic acid (twice daily) + barrier support (twice daily) + SPF

**Common combinations that crash:**

- AHA + retinoid same regimen
- Multiple acids (AHA + BHA daily) without recovery
- Daily exfoliating cleanser + active + scrub

## 5. Sensitised vs sensitive skin
...
- Sensitive skin needs gentle ongoing management (forever)
- Sensitised skin needs **active barrier repair** then can tolerate more in time
...
The barrier-repair conversation is critical to compliance.
...
- Pull three current clients with stalled plans. Is barrier dysfunction the actual driver?
